...
class Edit extends \Magento\Backend\App\Action
{
    ...
    protected function _initAction()
    {
        ...
        $resultPage->setActiveMenu(
            'Netbaseteam_Onlinedesign::googlefont_manage'
        )->addBreadcrumb(
            __('Onlinedesign'),
            __('Onlinedesign')
        )->addBreadcrumb(
    ...
    public function execute()
    {
        ...
        $id = $this->getRequest()->getParam('googlefont_id');
        $model = $this->_objectManager->create('Netbaseteam\Onlinedesign\Model\Googlefont');
        ...
        $this->_coreRegistry->register('googlefont', $model);

        // 5. Build edit form
		/** @var \Magento\Backend\Model\View\Result\Page $resultPage */
        $resultPage = $this->_initAction();
        $resultPage->addBreadcrumb(
            $id ? __('Edit Onlinedesign') : __('New Googlefont'),
            $id ? __('Edit Onlinedesign') : __('New Googlefont')
        );
        $resultPage->getConfig()->getTitle()->prepend(__('Onlinedesign'));
        $resultPage->getConfig()->getTitle()
            ->prepend($model->getId() ? $model->getTitle() : __('New Googlefont'));
        return $resultPage;
    }
}
