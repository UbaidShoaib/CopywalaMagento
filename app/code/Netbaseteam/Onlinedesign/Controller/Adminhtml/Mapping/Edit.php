...
class Edit extends \Magento\Backend\App\Action
{
    ...
    {
        return $this->_authorization->isAllowed('Netbaseteam_Onlinedesign::manage_template_mapping');
    }
    ...
    protected function _initAction()
    {
        ...
        $resultPage->setActiveMenu(
            'Netbaseteam_Onlinedesign::manage_template_mapping'
        )->addBreadcrumb(
            __('Onlinedesign'),
            __('Onlinedesign')
        )->addBreadcrumb(
            __('Manage Template Mapping'),
            __('Manage Template Mapping')
        );
        return $resultPage;
    }
    ...
    public function execute()
    {
        ...
        $model = $this->_objectManager->create('Netbaseteam\Onlinedesign\Model\Mapping');

        // 2. Initial checking
        if ($id) {
            $model->load($id);
            if (!$model->getId()) {
                $this->messageManager->addError(__('This template field no longer exists.'));
        ...
        $resultPage->addBreadcrumb(
            $id ? __('Edit Onlinedesign') : __('New Field'),
            $id ? __('Edit Onlinedesign') : __('New Field')
        );
