...
class Edit extends \Magento\Backend\App\Action
{
    ...
    protected function _initAction()
    {
        ...
        $resultPage->setActiveMenu(
            'Netbaseteam_Onlinedesign::color_manage'
        )->addBreadcrumb(
            __('Onlinedesign'),
            __('Onlinedesign')
        )->addBreadcrumb(
    ...
    public function execute()
    {
        ...
        $this->_coreRegistry->register('color', $model);

        // 5. Build edit form
		/** @var \Magento\Backend\Model\View\Result\Page $resultPage */
        $resultPage = $this->_initAction();
        $resultPage->addBreadcrumb(
            $id ? __('Edit Onlinedesign') : __('New Color'),
            $id ? __('Edit Onlinedesign') : __('New Color')
        );
        $resultPage->getConfig()->getTitle()->prepend(__('Onlinedesign'));
        $resultPage->getConfig()->getTitle()
            ->prepend($model->getId() ? $model->getTitle() : __('New Color'));
        return $resultPage;
    }
}
