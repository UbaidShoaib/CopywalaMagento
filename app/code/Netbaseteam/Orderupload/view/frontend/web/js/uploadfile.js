...
define([
    ...
    (function ($) {
        ...
        $.fn.uploadFile = function uploadFile(options) {
            ...
            this.createProgress = function (filename,filepath,filesize, comment) {
                ...
                if(s.showComment) {
                    ...
                    if(pd.comment.length <= 0){
                        pd.comment.insertAfter(".ajax-file-upload-container .ajax-file-upload-statusbar > img");
                    }
                    ...
                    pd.done.click(function () {
                        ...
                        if(pd.msgcomment != null && pd.msgcomment != "") {
                            var src_path = pd.preview.attr('src').split(s.parent_id);
                            var src_name = src_path[1];
                            var imgPreview = src_name.substring(1, src_name.length);/* get image */
                            uploadCommentByAjax(s.parent_id, s.comment_updurl, s.json_file, pd.msgcomment, pd.done, imgPreview);
                        } else {
                            pd.comment.focus();
            ...
            function uploadCommentByAjax(parent_id, comment_updurl, json_file, msgcomment, btnComment, imgPreview){
                ...
                $.ajax({
                    ...
                    data: {
                        ...
                        child_id: "",
                        json_file: json_file,
                        file: parent_id + "/" + imgPreview /* ex: 163/image.jpg */,
                        msg: msgcomment,
            ...
            function serializeAndUploadFiles(s, obj, files) {
                ...
                for(var i = 0; i < files.length; i++) {
                    ...
                    if($.inArray(ext, allow_file_type) == -1){
                        $("<div class='" + s.errorClass + "'><b>" + files[i].name + "</b> not allow this extention " + ext + "</div>").appendTo(
                            obj.errorLog);
                        continue;
                    }
            ...
            function createCustomInputFile (obj, group, s, uploadLabel) {
                ...
                var id = "upload-file-nb";
                var form = $("<form method='" + s.method + "' id='"+ id + "' action='" + s.url + "' enctype='" + s.enctype + "'></form>");
            ...
            function defaultProgressBar(obj,s)
            {
                ...
                if(s.showComment){
                    this.done.addClass("icon-arrows-ccw");
                } else {
                ...
                this.comment = $("<textarea rows='3' cols='55' class='orderupload-comment'></textarea>").appendTo(this.statusbar).hide();

                this.abort.addClass("icon-trash");
                /* this.done.addClass("icon-download"); */

                this.download.addClass("icon-download");
                this.cancel.addClass("icon-trash");
                this.del.addClass("icon-trash");
                ...
                var options = {
                    ...
                    success: function (data, message, xhr) {
                        ...
                        if(s.showStatusAfterSuccess) {
                            ...
                            if(s.showComment) {
                                ...
                                pd.done.click(function () {
                                    ...
                                    pd.msgcomment = pd.comment.val();
                                    if(pd.msgcomment != null && pd.msgcomment != "") {
                                        var imgPreview = data[0] /* get image for preview */;
                                        uploadCommentByAjax(s.parent_id, s.comment_updurl, s.json_file, pd.msgcomment, pd.done, imgPreview);
                                    } else {
                                        pd.comment.focus();
                                        alert("Please enter you comment message");
                        ...
                        if(s.showComment) {
                            ...
                            if( $('.ajax-file-upload-container .ajax-file-upload-statusbar .icon-arrows-ccw').length <= 0){
                                pd.done.insertBefore(".ajax-file-upload-container .ajax-file-upload-statusbar .icon-trash");
                            }
                            if( $('.ajax-file-upload-container .ajax-file-upload-statusbar .icon-arrows-ccw').css('display') == 'none'){
                                $('.ajax-file-upload-container .ajax-file-upload-statusbar .icon-arrows-ccw').css('display', 'block');
                            }
                        }
                        ...
                        $(".nb_orderupload #label_oderupload div.error").length ? $(".nb_orderupload div.error").html("") : '';
                        $(".nb_orderupload .fileuploader-box.error-upload-button").length ? $(".nb_orderupload .fileuploader-box.error-upload-button").removeClass("error-upload-button") : '';
                    },
